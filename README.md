...
In order to facilitate modeling of human-centric activities like shifts in industrial processes or energy systems, the `DateTime.DateTimeSystem` allows to define a set of holidays, which are ignored if a `Schedule` component is defined and `workdays` set as the `repetition` value. This repetition will also only trigger on what are considered workdays, which is defined DateTime-wide in `DateTime.Data.WorkDays` to be Monday, Tuesday,...,Friday. If another choice is desired, this would have to be changed in this record.
...
The other important example is how to use the DateTime library to model repeating schedules, like e.g. from industrial work shifts or discrete production processes. 
