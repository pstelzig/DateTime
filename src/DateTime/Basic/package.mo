...
model in which SimWeekday is used.</p>
  ...
  algorithm
    when initial() then
      ...
      (prevTrigger, prevTriggerPosix) := DateTime.Functions.largestPreviousTrigger(dateTimeSystem.startPosix + time, initTrigger, repetition, triggerTimezone, dateTimeSystem.holidays, dateTimeSystem.withLeapSeconds, autocorrect, strategy);
    end when;
    ...
    when repetition <> "" and simTimePosix >= prevTriggerPosix + onTime then
      prevTrigger := DateTime.Functions.addSingleRepetitionToDatetime(prevTrigger, repetition, triggerTimezone, dateTimeSystem.holidays, dateTimeSystem.withLeapSeconds, autocorrect, strategy);
